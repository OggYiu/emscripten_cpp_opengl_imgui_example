...
void ShowMyError(const char* label)
{
...
int Atlas::LoadImage(const char* filename) {
  ...
  if (surface == nullptr) { ShowMyError("Unable to load image"); }
...
SDL_Surface* Atlas::GetSurface() {
  if (self->atlas == nullptr) {
    ...
    for (unsigned i = 0; i < self->sources.size(); i++) {
      if (!rects[i].was_packed) { ShowMyError("Could not fit all images"); }
